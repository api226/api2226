...
parser = OptionParser
parser.add_option("--collect-only",action="store_true",dest="collect_only",help="仅搜集测试用例,不执行测试")
parser.add_option("--makesuite-tag",action="store_true",dest="makesuite-tag",help="根据标签生成测试套件")
parser.add_option("--rerun_fails",dest="rerun-fails",action="store_true",help="重新运行失败案例")
#生效参数
